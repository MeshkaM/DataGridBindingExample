...
namespace MaterialDesignFixedHintTextBox
{
    public class DAL
    {
        ...
        public static async Task<List<PlacesOfInterest>> LoadPlacesOfInterestAsync()
        {
            using (IDbConnection conn = new SqlConnection(ConnString))
            {
                var result = await conn.QueryAsync<PlacesOfInterest>("SELECT * FROM PlacesOfInterest");
